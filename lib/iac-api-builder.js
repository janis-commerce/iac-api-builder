...
// Promisify Methods
['stat', 'copyFile', 'appendFile', 'readFile', 'mkdir'].forEach(method => {
...
class IacApiBuilder {
	...
	static get buildDirectory() {
		return path.join(process.cwd(), IAC_DIRECTORY, BUILD_DIRECTORY);
	}
	 ...
	 */
	async initBuiltFile() {
		await fs.mkdir(this.constructor.buildDirectory, { recursive: true });
	...
	async appendToTemplate(resource) {
		...
		stringResource += YAML.stringify(resource).replace(/[|]-\s+/g, '');
		// Add correct Identation
		stringResource = stringResource.replace(/[\n\r]/g, '\n  ');
		await fs.appendFile(this.constructor.buildFilePath, stringResource);
	}
