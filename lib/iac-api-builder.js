...
// Promisify Methods
['stat', 'copyFile', 'appendFile'].forEach(method => {
...
class IacApiBuilder {
	...
	async build() {
		...
		try {
			...
			return logger(`Can't copy Files. ${error.message}`, 'ERROR', true);
		}
		...
		try {
			...
			return logger(`Can't build Files. ${error.message}`, 'ERROR', true);
		}
